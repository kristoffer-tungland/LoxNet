...
Important Notes
Adoption to getIp-Handling may be required. See Support for distributed data
! Since May 2021 Remote Connect supports globally distributed datacenters.
centers
used (on Miniservers of the new generation). This allows for clients to be
! Since Version 11.2 application layer encryption is no longer mandatory if TLS is
implemented without the hassle of having to implement our application layer
encryption.
! Since November 2019, our new generation of the Miniserver has been
introduced. The new generation supports TLS Protocols such as WSS and HTTPS.
For details see chapter Using HTTPS/WSS
introduced. Starting with future versions, passwords will be hashed using
! As of Loxone Config 10.3 new hashing algorithms for passwords have been
SHA256. In order to ensure compatibility for future versions, please make sure to
follow the updated instructions listed here.
16.0
Communicating with the Miniserver Page 3 of 36
! As of Loxone Config 10.2 JSON Web Tokens (https://jwt.io/) have been
introduced. Using legacy tokens is still supported, but deprecated in version 10.2
and will be removed in future versions. In order to ensure compatibility for future
versions, please make sure to follow the updated instructions listed here.
!
...
Note: Using HTTP Basic Authentication for HTTP Requests is still available for
debugging and testing purposes. Since credentials passed along using basic
authentication may be intercepted and compromised, it is strongly recommended
to use token based authentication.
This document has been revised and only contains info on token based
authentication. Please make sure your implementation is ready for tokens.
16.0
...
4. 5. Generate a AES256-CBC key -> {key} (Hex)
...
a. b. “{key}:{iv}” is the payload that needs to be encrypted using RSA
This key and iv will be used to AES-encrypt messages on the websocket (e.g. for
...
key}”
8. a. 9. Generate a random salt, hex string (length may vary, e.g. 2 bytes) -> {salt}
This salt will be used to AES-encrypt messages on the websocket (e.g. for
authentication or token acquisition). More info on encrypted commands.
...
HTTPS basically is HTTP, but TLS (Transport Layer Security) is used for communication. The same
...
1. 2. Request the current IP & Port from the CloudDNS
Create a hostname containing the IP and port returned by the IPHTTPS attribute of the
response.
a. b. Split up IPHTTPS into the {ip} and {port}
...
“dns.loxonecloud.com/?getip&snr={ SNR of Miniserver }&json=true”. The Request returns the current
...
Should a Miniserver unknown by a previously datacenter (Code 405), fall back to step #1
6. Controls
...
1. 2. request the visualization password from the user - {visuPw}
request a {key}, {salt} and the used hashing algorithm {hashAlg} from the Miniserver
(“jdev/sys/getvisusalt/{user}”)
a. {user} = the user whos visu password has been entered
3. 4. Create an {hashAlg} hash (SHA1, SHA256,..) of “{visuPw}:{salt}” -> {visuPwHash}
Create an HMAC-SHA1 or HMAC-SHA256 hash using the uppercase {visuPwHash} and the
...
1. 2. the key from the “jdev/sys/getkey”, “jdev/sys/getkey2” or “jdev/sys/getvisusalt” responses are
hex-encoded and might need to be converted to ASCII before being used to create a hash.
create a hash from the desired text (user:passHash, visuPwHash, token) using HMAC-SHA1
...
,
Text-
, Daytimer- and Weather-States. These Event-Tables contain the current states of all sensors
and actuators.
...
5 Out-Of-Service Indicator - presumably due to an Firmware-Update. No message is
going to follow this header, the Miniserver closes the connection afterwards, the
client may try to reconnect.
6 Keepalive response (after sending “keepalive”, the Miniserver will respond with this
identifier - therefore the connection is up and running!)
...
// 64-Bit Float (little endian) value
double dVal; } PACKED EvData;
...
unsigned long Data1; unsigned short Data2; unsigned short Data3; unsigned char Data4[8]; } PACKED PUUID;
// 32-Bit Unsigned Integer (little endian)
// 16-Bit Unsigned Integer (little endian)
// 16-Bit Unsigned Integer (little endian)
// 8-Bit Uint8Array [8] (little endian)
...
AES256 and Public-key cryptography for the AES session key exchange.
...
6. 7. 8. Generate a random AES iv (16 byte) -> {iv} (Hex)
Encrypt the {plaintext} with AES {key} + {iv} -> {cipher} (Base64)
URI-Component-Encode the {cipher} -> {enc-cipher}
...
If it cannot be decrypted (invalid public key, unexpected salt change) it will return
401
